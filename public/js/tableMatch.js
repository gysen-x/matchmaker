...
table?.addEventListener("click", async (event) => {
  ...
  if (event.target.classList.contains("cancel-button")) {
    ...
    if (result.message === "Entry and Match has been deleted") {
      ...
      const counterCell = event.target
        .closest(".table-row")
        .querySelector(`[data-matchcounter="${match_id}"]`);
      console.log("counterCell: ", counterCell.innerText.split("/"));
      const counter = counterCell.innerText;
      counterCell.innerText = `${Number(counter.split("/")[0]) - 1} / ${
        counter.split("/")[1]
      }`;
      console.log("counterCell: ", counterCell.innerText.split("/"));
    }
  ...
  if (event.target.classList.contains("join-button")) {
    modalConfirm.querySelector(".join-button").dataset.matchid =
      event.target.closest(".table-row").dataset.matchid;
    modalConfirm.querySelector(".join-button").dataset.userid =
      document.getElementById("user-account").dataset.userid;
  ...
  if (event.target.classList.contains("join-button")) {
    ...
    try {
      ...
      if (!result.message) {
        modalConfirm.style.display = "none";
        alert("Приятной игры!");
        const matchPlayersBox = table
          .querySelector(`[data-matchid="${match_id}"]`)
          .querySelector("[data-matchcounter]");
        const matchPlayers = matchPlayersBox.textContent.split("/");
        matchPlayersBox.innerText = `${Number(matchPlayers[0]) + 1} / ${
          matchPlayers[1]
        }`;
      }
    } catch (error) {
